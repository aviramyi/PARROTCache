...
from policy_learning.cache_model import attention
from policy_learning.cache_model import embed
from policy_learning.cache_model import loss as L
from policy_learning.cache_model import utils
...
class EvictionPolicyModel(nn.Module):
    ...
    def from_config(cls, config):
        ...
        if config.get("cache_line_embedder") == "address_embedder":
            ...
            cache_line_embedder = embed.from_config(
                config.get("cache_line_embedder"))
        ...
        if cache_pc_embedder_type == "none":  # default no pc embedding
            ...
            cache_pc_embedder = embed.from_config(
                config.get("cache_pc_embedder"))

        positional_embedder = embed.from_config(
            config.get("positional_embedder"))
    ...
    def forward(self, cache_accesses, prev_hidden_state=None, inference=False):
        ...
        pred_reuse_distances = self._reuse_distance_estimator(
            context).squeeze(-1)
        ...
        next_hidden_state = (
            (next_c, next_h), hidden_state_history, access_history)
    ...
    def loss(self, eviction_traces, warmup_period):
        ...
        for i in range(warmup_period):
            cache_accesses = [
                trace[i].cache_access for trace in eviction_traces]
        ...
        for i in range(warmup_period, len(eviction_traces[0])):
            ...
            for name, loss_fn in self._loss_fns.items():
                loss = loss_fn(scores, pred_reuse_distances,
    ...
    def _initial_hidden_state(self, batch_size):
        ...
        initial_cell_state = torch.zeros(
            batch_size, self._lstm_cell.hidden_size)
...
class ApproxNDCGLoss(LossFunction):
    ...
    def __init__(self):
        super().__init__()
        logging.warning(
            "Expects that all calls to loss are labeled with Belady's")
